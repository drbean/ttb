...
# Last Edit: 2005 Nov 21, 07:38:16 AM
	...
	foreach my $player ( @{$players} )
	{
		...
		foreach my $glyph ( @{$text[$playerN]} )
		{
			...
			elsif ( $text[$playerN][$letterN][2] =~ $letter and $inword == 0 )
			{
				...
				$blankedText{$player} =
					join '', $blankedText{$player},
				'\begin{footnotesize}'. ($wordN+1) . '\end{footnotesize}\x';
				$inword = 1;
		...
		$quiz->{"dic$player"} = '';
		$quiz->{"score$player"} = $blanks - $blanksAlloted{$player};
...
\\mycard{$latex{role}[$paperPosition]}
{$table}
{";
		my $line;
