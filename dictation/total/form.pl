...
# Last Edit: 2007 Jun 15, 03:53:37 PM
...
if ( -e '../assistants.yaml' )
{
	...
	$assistants = $assistantFile->{ $round->{round} };
}
...
$tmplHash->{assistant} = [keys %$assistants] if $assistants;
@{$tmplHash->{assistantId}} = map { $ids{$_} } keys %$assistants if $assistants;
# @{$tmplHash->{assistant}} = @assistants;
