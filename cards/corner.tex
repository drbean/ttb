\documentclass[a4paper,landscape]{article}
\usepackage{fontspec}
\setmainfont{Linux Libertine O}[
	Scale=MatchLowercase,
	Scale = 2.0,
]
\usepackage{xeCJK}
\setCJKmainfont{NotoSansCJK-Regular.ttc}[
	Path = /usr/share/fonts/noto/,
]
% \usepackage[absolute,noshowtext,showboxes]{textpos}
\usepackage[absolute]{textpos}
% \textblockorigin{-0.02cm}{0.07cm} %HPDeskJet5160
% \textblockorigin{0.00cm}{0.00cm} %HPDeskJet5160
\textblockorigin{0.00cm}{0.00cm} %HPDeskJet5160
% \textblockorigin{0.05cm}{0.13cm} %HPDeskJet5160
% \textblockorigin{0.00cm}{0.00cm} %HPLaserJet5000LE
\usepackage[none]{hyphenat}
\usepackage{graphicx}
\graphicspath{ {/home/$ENV{USER}/ttb/forms/bean/} }
\pagestyle{empty}
\setlength{\unitlength}{1cm}

\setlength{\TPVertModule}{2.1214cm}

\newcommand{\myCornerIdentifier}[0]{
corner
}

\newcommand{\myTopRightHandCorner}[0]{
	\Huge Top Right Hand Corner
% \includegraphics[angle=00,height=0.07\paperheight,width=0.24\paperwidth]{five_bean.png}
}

\newcommand{\myTopLeftHandCorner}[0]{
	\Huge Top Left Hand Corner
% \includegraphics[angle=00,height=0.07\paperheight,width=0.24\paperwidth]{five_bean.png}
}

\newcommand{\myBottomRightHandCorner}[0]{
	\Huge Bottom Right Hand Corner
% \includegraphics[angle=00,height=0.07\paperheight,width=0.24\paperwidth]{five_bean.png}
}

\newcommand{\myBottomLeftHandCorner}[0]{
	\Huge Bottom Left Hand Corner
% \includegraphics[angle=00,height=0.07\paperheight,width=0.24\paperwidth]{five_bean.png}
}

\newcommand{\myTopMiddle}[0]{
	\Huge In the \mbox{Middle} at the Top
% \includegraphics[angle=00,height=0.07\paperheight,width=0.24\paperwidth]{five_bean.png}
}

\newcommand{\myRightMiddle}[0]{
	\flushleft \Huge In the \mbox{Middle} on the Right
% \includegraphics[angle=00,height=0.07\paperheight,width=0.24\paperwidth]{five_bean.png}
}

\newcommand{\myBottomMiddle}[0]{
	\flushleft \Huge In the \mbox{Middle} at the \mbox{Bottom}
% \includegraphics[angle=00,height=0.07\paperheight,width=0.24\paperwidth]{five_bean.png}
}

\newcommand{\myLeftMiddle}[0]{
	\flushleft \Huge In the \mbox{Middle} on the Left
% \includegraphics[angle=00,height=0.07\paperheight,width=0.24\paperwidth]{five_bean.png}
}

\newcommand{\myMiddle}[0]{
	\Huge In the \mbox{Middle}
% \includegraphics[angle=00,height=0.07\paperheight,width=0.24\paperwidth]{five_bean.png}
}

% \newcommand{\myDcontent}[0]{
% 
% }

\newcommand{\mycard}[5]{%
	\vspace{0.3cm}
	\tiny #1 #2
	\vspace{-0.03cm}
	\par
	\parbox[t][4.7cm][c]{5.3cm}{%
	\hspace{-0.8cm} \large#3\\
	}
}

\begin{document}
% \begin{picture}(15,20)(+4.8,-22.05)
% \begin{tabular}[t]{*{2}{|p{10.05cm}}|}

\begin{textblock}{5}(1,1)
\textblocklabel{picture1}
\mycard{}{\myCornerIdentifier}{
\myTopLeftHandCorner
}{}{} 
\end{textblock}

\begin{textblock}{5}(6,1)
\textblocklabel{picture2}
\mycard{}{\myCornerIdentifier}{
\myTopMiddle
}{}{} 
\end{textblock}

\begin{textblock}{5}(11,1)
\textblocklabel{picture1}
\mycard{}{\myCornerIdentifier}{
\myTopRightHandCorner
}{}{} 
\end{textblock}

\begin{textblock}{5}(1,4)
\textblocklabel{picture2}
\mycard{}{\myCornerIdentifier}{
\myLeftMiddle
}{}{} 
\end{textblock}

\begin{textblock}{5}(6,4)
\textblocklabel{picture3}
\mycard{}{\myCornerIdentifier}{
\myMiddle
}{}{} 
\end{textblock}

\begin{textblock}{5}(11,4)
\textblocklabel{picture5}
\mycard{}{\myCornerIdentifier}{
\myRightMiddle
}{}{} 
\end{textblock}

\begin{textblock}{5}(1,7)
\textblocklabel{picture3}
\mycard{}{\myCornerIdentifier}{
\myBottomLeftHandCorner
}{}{} 
\end{textblock}

\begin{textblock}{5}(6,7)
\textblocklabel{picture5}
\mycard{}{\myCornerIdentifier}{
\myBottomMiddle
}{}{} 
\end{textblock}

\begin{textblock}{5}(11,7)
\textblocklabel{picture3}
\mycard{}{\myCornerIdentifier}{
\myBottomRightHandCorner
}{}{} 
\end{textblock}

\begin{tiny}1\end{tiny}\newpage

\end{document}
